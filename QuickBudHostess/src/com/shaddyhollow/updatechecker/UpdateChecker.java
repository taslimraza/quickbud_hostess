package com.shaddyhollow.updatechecker;
...
public class UpdateChecker {
	 ...
	 * 
	 * @param context
	 *            An instance of your Activity's context
	 * @since API 1
	 */
	 ...
	 *            An instance of your Activity's context
	 * @param toasts
	 *            True if you want toast notifications, false by default
	 * @since API 2
	 */
	 ...
	 * 
	 * @param url
	 *            URL at which the text file containing your latest version code
	 *            is located.
	 * @since API 1
	 */
	public void checkForUpdateByVersionCode(String url) {
		if (isOnline()) {
				...
				if (versionCode >= 0) {
					try {
						...
						if (readCode > versionCode) {
							updateAvailable = true; // We have an update
													// available
						}
					} catch (NumberFormatException e) {
						Log.e(TAG, "Invalid number online"); // Something wrong
																// with the file
																// content
					}

				} else {
					Log.e(TAG, "Invalid version code in app"); // Invalid
																// version code
				}
			} else {
			...
			if (useToasts) {
				makeToastFromString(
						"App update check failed. No internet connection available")
						.show();
			}
		}
	}
	 ...
	 * 
	 * @param apkUrl
	 *            URL at which the update is located
	 * @since API 1
	 */
	public void downloadAndInstall(String apkUrl) {
		if (isOnline()) {
			...
			if (useToasts) {
				makeToastFromString(
						"App update failed. No internet connection available")
						.show();
			}
		}
	}
	...
	public void launchMarketDetails() {
		if (haveValidContext) {
			if (hasGooglePlayInstalled()) {
				...
				if (useToasts) {
					makeToastFromString(
							"Google Play isn't installed on your device.")
							.show();
				}
			}
		}
	...
	public boolean hasGooglePlayInstalled() {
		Intent market = new Intent(Intent.ACTION_VIEW,
				Uri.parse("market://details?id=dummy"));
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i).activityInfo.packageName
						.startsWith("com.android.vending") == true) {
					return true;
				}
			}
		}
	 ...
	 * 
	 * @param text
	 *            The text to be displayed by the toast
	 ...
	 * 
	 * @param url
	 *            The URL at which the file is located
